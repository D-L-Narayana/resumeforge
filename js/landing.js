...
$('pricingGrid').innerHTML=D.pricing.map(function(p){return `<div class="card price-card reveal ${p.popular?'popular':''}">${p.popular?'<span class="pop-tag">MOST POPULAR</span>':''}<h3>${p.name}</h3><div class="price">${p.price}<small>${p.per}</small></div><ul class="plist">${p.features.map(function(f){return `<li>${f}</li>`;}).join('')}</ul><a class="btn ${p.popular?'btn-primary':'btn-ghost'}" href="${p.name==='Free'?'builder.html':('mailto:dlnarayana2005@gmail.com?subject='+encodeURIComponent('ResumeForge '+p.name+' plan')+'&body='+encodeURIComponent('Hi! I would like to get the ResumeForge '+p.name+' plan ('+p.price+(p.per||'')+'). Please share the payment details.'))}">${p.cta}</a></div>`;}).join('');
